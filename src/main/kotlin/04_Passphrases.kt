...
fun main(args: Array<String>) {
    val passphrasesWords = input.split("\n")
            .map { it.split(" ") }
    val valid = passphrasesWords.filter { it.size == it.toSet().size }.count()
    println(valid)

    val valid2 = passphrasesWords.map { it.map { it.toSortedSet().joinToString("") } }
